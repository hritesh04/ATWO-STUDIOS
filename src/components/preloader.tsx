...
export default function Preloader({ onComplete }: { onComplete: () => void }) {
  const [phase, setPhase] = useState<"start" | "loading" | "expand" | "done">(
    "start"
  );

  useEffect(() => {
    const loadTimer = setTimeout(() => setPhase("loading"), 600);
    const expandTimer = setTimeout(() => setPhase("expand"), 1400);
